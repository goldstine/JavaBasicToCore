...
/**
 * 测试newScheduledTreadPool线程池的 schedule()   再测试scheduleAtFixedRate()
 */
...
//        es.schedule(new MyTaskRunnable2(1),2, TimeUnit.SECONDS);
        ...
        es.scheduleAtFixedRate(new MyTaskRunnable2(1),1,2,TimeUnit.SECONDS);//任务的执行时间包含在间隔时间中
...
//        es.schedule(new MyTaskRunnable2(1),2, TimeUnit.SECONDS);
        ...
        //提交多个任务
        for (int i = 0; i < 10; i++) {
            es.schedule(new MyTaskRunnable2(i),2, TimeUnit.SECONDS);
        }
...
class MyTaskRunnable2 implements Runnable{

    private int id;

    public MyTaskRunnable2(int id) {
        this.id = id;
    }
